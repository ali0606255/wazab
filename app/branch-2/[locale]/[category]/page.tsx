...
/** One prerendered page per branch-2 category per locale. */
export async function generateStaticParams() {
  const params = await Promise.all(
    LOCALES.map(async (locale) => {
      const ids = await getCategoryIds(locale, BRANCH);
...
export async function generateMetadata({
  params,
}: PageProps<"/branch-2/[locale]/[category]">): Promise<Metadata> {
  const { locale, category: categoryId } = await params;
  if (!isLocale(locale)) return {};

  const category = await getCategory(locale, categoryId, BRANCH);
  if (!category) return {};
  ...
  return {
    ...
    alternates: {
      canonical: `${BASE_PATH}/${locale}/${categoryId}`,
      languages: {
        ar: `${BASE_PATH}/ar/${categoryId}`,
        en: `${BASE_PATH}/en/${categoryId}`,
      },
    },
  };
}

export default async function Branch2CategoryPage({
  params,
}: PageProps<"/branch-2/[locale]/[category]">) {
  const { locale, category: categoryId } = await params;
